...
The script reproduces the main computational pipeline described in the manuscript:
    ...
    4. Fine alignment using Multi-population Differential Evolution (MDE).
    5. Repeated experiments, descriptive statistics, and convergence plots.
...
class Config:
    ...
    mde_trim_ratio: float = 0.90
    mde_early_stop_patience: int = 30
    mde_jobs: Optional[int] = None
    mde_min_rotation_range_degrees: float = 5.0
...
def build_adaptive_bounds(
    ...
    rotation_range = max(rotation_range, config.mde_min_rotation_range_degrees)
...
def mde_registration(
    ...
    """
    Karkinli-style MDE with adaptive bounds, trimmed fitness, parallel evaluation,
    and early stopping.
    """
    if seed is not None:
        np.random.seed(seed)
...
def save_summary_outputs(
    ...
    """Save tables and figures for the MDE fine-registration runs."""
    ...
    summary_table = pd.DataFrame(
        {
            ...
            "Median (m)": [
                results_df["rmse_initial"].median(),
                results_df["rmse_global"].median(),
                results_df["rmse_mde"].median(),
            ],
            "Std. Dev. (m)": [
    ...
    summary_table.to_csv(os.path.join(tables_dir, "rmse_summary.csv"), index=False)
    with open(os.path.join(tables_dir, "rmse_summary.tex"), "w", encoding="utf-8") as handle:
    ...
    x_mde = np.random.normal(loc=1.0, scale=0.03, size=len(results_df))
    plt.scatter(x_mde, results_df["rmse_mde"].values, s=18, alpha=0.8)
    plt.ylabel("RMSE (m)")
    plt.title(f"Distribution of MDE RMSE Values ({len(results_df)} Runs)")
    plt.tight_layout()
    ...
    plt.title(f"MDE Convergence (Average of {len(results_df)} Runs)")
    plt.legend()
    plt.tight_layout()
...
def run_single_experiment(
    ...
    print("\nStep 2: Fine alignment with MDE")
    start_mde = time.time()
...
def run_all_experiments(config: Config) -> pd.DataFrame:
    ...
    save_summary_outputs(results_df, mde_histories, directories, config)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--mde_population_size", type=int, default=10, help="MDE sub-population size")
    parser.add_argument("--mde_max_cycle", type=int, default=250, help="Maximum number of MDE cycles")
...
def build_config_from_args(args: argparse.Namespace) -> Config:
    ...
    return Config(
        ...
        mde_population_size=args.mde_population_size,
        mde_max_cycle=args.mde_max_cycle,
